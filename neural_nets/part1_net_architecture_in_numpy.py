...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            hidden_layer_output = self.hidden_layer.forward(x_data)
            y_pred = self.output_layer.forward(hidden_layer_output)
            return y_pred
    ...
    model.hidden_layer.W[:, 0] = [0.8, 0.2]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-0.7, -0.2]      # wagi neuronu h2
    model.hidden_layer.b[:] = [-0.4, 0.6]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [0.5,0.5]      # wagi neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193609         # Twój numer indeksu, np. 102247
