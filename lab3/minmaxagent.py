...
class MinMaxAgent:
    def __init__(self, my_token, heuristic = True,depth=3):
    ...
    def decide(self, connect4):
        ...
        for drop in connect4.possible_drops():
            ...
            value = self.minimax(connect4_copy, False, self.depth)
    ...
    def minimax(self, state, maximizingPlayer, depth):
        if state.game_over:
            ...
            for drop in state.possible_drops():
                ...
                value = max(value, self.minimax(state_copy, False, depth-1))
            ...
            for drop in state.possible_drops():
                state_copy = copy.deepcopy(state)
                state_copy.drop_token(drop)
                value = min(value, self.minimax(state_copy, True, depth-1))
